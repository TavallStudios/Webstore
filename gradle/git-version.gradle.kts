...
val exact = git("tag", "--points-at", "HEAD").lineSequence().firstNotNullOfOrNull { pattern.matchEntire(it) }
val latest = git("for-each-ref", "--merged=HEAD", "--sort=-creatordate", "--format=%(refname:short)", "refs/tags")
    .lineSequence().firstNotNullOfOrNull { pattern.matchEntire(it) }
val numeric = when {
    exact != null -> exact.groupValues.drop(1).joinToString(".")
    latest != null -> latest.groupValues.drop(1).map(String::toInt).let { "${it[0]}.${it[1]}.${it[2] + 1}" }
